...
CREATE TABLE TTN_GATEWAYS(
	...
	CONSTRAINT FK_TTN_GATEWAYS_TTN_LOCATIONS FOREIGN KEY (location_guid)
		REFERENCES TTN_LOCATIONS (location_guid)
		ON DELETE CASCADE
		ON UPDATE CASCADE
);

CREATE TABLE TTN_SENSORS(
...
CREATE TABLE TTN_DEVICES(
	...
	CONSTRAINT FK_TTN_DEVICES_TTN_APPLICATIONS FOREIGN KEY (application_guid)
		REFERENCES TTN_APPLICATIONS (application_guid)
		ON DELETE CASCADE
		ON UPDATE CASCADE
);

CREATE TABLE TTN_UPLINKS(
	...
	CONSTRAINT FK_TTN_UPLINKS_TTN_DEVICES FOREIGN KEY (device_guid)
		REFERENCES TTN_DEVICES (device_guid)
		ON DELETE CASCADE
		ON UPDATE CASCADE,
	CONSTRAINT FK_TTN_UPLINKS_TTN_WARNINGS FOREIGN KEY (warning_guid)
		REFERENCES TTN_WARNINGS (warning_guid)
		ON DELETE CASCADE
		ON UPDATE CASCADE
);
...
CREATE TABLE TTN_RX(
	...
	CONSTRAINT FK_TTN_RX_TTN_GATEWAY FOREIGN KEY (gateway_guid)
		REFERENCES TTN_GATEWAYS (gateway_guid)
		ON DELETE CASCADE
		ON UPDATE CASCADE,
	CONSTRAINT FK_TTN_RX_TTN_UPLINKS FOREIGN KEY (uplink_guid)
		REFERENCES TTN_UPLINKS (uplink_guid)
		ON DELETE CASCADE
		ON UPDATE CASCADE
);

CREATE TABLE TTN_HOPS(
	...
	CONSTRAINT FK_TTN_HOPS_TTN_RX FOREIGN KEY (rx_guid)
		REFERENCES TTN_RX (rx_guid)
		ON DELETE CASCADE
		ON UPDATE CASCADE
);

CREATE TABLE TTN_DATETIMES(
...
CREATE TABLE TTN_CORRELATION_IDS(
...
CREATE TABLE TTN_UPLINK_TOKENS(
	rx_guid UNIQUEIDENTIFIER NOT NULL,
	gateway_guid UNIQUEIDENTIFIER NOT NULL,
...
CREATE TABLE TTN_READINGS(
	...
	CONSTRAINT FK_TTN_READINGS_TTN_SENSORS FOREIGN KEY (sensor_guid)
		REFERENCES TTN_SENSORS (sensor_guid)
		ON DELETE CASCADE
		ON UPDATE CASCADE
);

CREATE TABLE TTN_UPLINK_SETTINGS(
